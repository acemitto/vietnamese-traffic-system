...
model2 = YOLO("yolov9e.pt")  # load a pretrained model (recommended for training)
...
filename="/Users/acemitto/Desktop/Test YOLOv9v10.mp4"
...
def main(sess=''):
	if True:
		...
		print("v10")
		results2 = model2(frame, device="mps")[0]
		...
		detections2 = detections2[np.isin(detections2.class_id, selected_classes)]
		...
		for xyxy, conf, cls in zip(detections2.xyxy, detections2.confidence, detections2.class_id):
			x_c, y_c, bbox_w, bbox_h = xyxy_to_xywh(*xyxy)
			xywh_obj = [x_c, y_c, bbox_w, bbox_h]
			xywh_bboxs.append(xywh_obj)
			confs.append([conf.item()])
