...
def estimate_speed(location1, location2, obj_name):
...
def getDataCrossedTheLight(id, data_deque, object_speed, obj_name):
    if data_deque[1][1]>yl2 and data_deque[0][1]<yl2:
        ...
        location = 'Tran Nhat Duat, Pho Co Ha Noi, Hoan Kiem, Ha Noi'
        vehicle_type = str(obj_name)
        vehicle_speed = str(object_speed)
        data_signal = 'OK'
        
        if csv_line != 'not_available':
            # STT| Thời gian| Vị Trí| Loại xe| Tốc độ (km/h)| Trạng thái dữ liệu
            with open('/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/vietnamese-traffic-system/Source/Traffic-Rule-Violation-Detection-System-master/Rule Breakers/CSV/traffic_measurement_'+datetime.today().strftime('%Y%m%d')+'.csv', 'a') as f:
                writer = csv.writer(f)
                csv_line_queue = status+'| '+time_obj+'| '+location+'| '+vehicle_type+'| '+vehicle_speed+'| '+data_signal
                writer.writerows([csv_line_queue.split('|')])


stt_violation = 0
def getDataViolation(object_speed, obj_name, name, license_plate_queue):
    global stt_violation
    stt_violation = stt_violation + 1
    status = str(stt_violation)
    time_obj = str(time.asctime(time.localtime(time.time())))
    license_plate = license_plate_queue
    location = 'Tran Nhat Duat, Pho Co Ha Noi, Hoan Kiem, Ha Noi'
    vehicle_type = str(obj_name)
    vehicle_speed = str(object_speed)
    violation = 'Over Speed (60km/h)'
    traffic_light = 'NaN'
    file_evidence_path = str(name)
    data_signal = 'OK'
    
    if csv_line != 'not_available':
        # STT| Thời gian| Biển số| Vị Trí| Loại xe| Tốc độ (km/h)| Vi phạm| Giá trị đèn| Ảnh| Trạng thái dữ liệu
        with open('/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/vietnamese-traffic-system/Source/Traffic-Rule-Violation-Detection-System-master/Rule Breakers/CSV/violation_'+datetime.today().strftime('%Y%m%d')+'.csv', 'a') as f:
            writer = csv.writer(f)
            csv_line_queue = status+'| '+time_obj+'| '+license_plate+'| '+location+'| '+vehicle_type+'| '+vehicle_speed+'| '+violation+'| '+traffic_light+'| '+file_evidence_path+'| '+data_signal
            writer.writerows([csv_line_queue.split('|')])

# kiểm tra vận tốc
def  checkSpeed(kmh,bimg, obj_name, data_deque):
    if data_deque[1][1]>yl2 and data_deque[0][1]<yl2:
        if kmh >= 60: # giới hạn vận tốc
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if len(data_deque[id]) >= 2:
            object_speed = estimate_speed(data_deque[id][1], data_deque[id][0], obj_name)
...
model = yolov7.load("yolov7.pt",trace=False)#),device="cuda:0")
print("model loaded")
...
polygons = [ np.array([
    [387, 288],[900, 288],[1162, 720],[74, 720]
])]
polygons2 = [ np.array([
    [0, 350],[1280, 350],[1280, 710],[0, 710]
])]

cap=cv2.VideoCapture('/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/vietnamese-traffic-system/Source/Traffic-Rule-Violation-Detection-System-master/IMG_4659.MP4') # 0 stands for very first webcam attach
...
filename="/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/vietnamese-traffic-system/Source/Traffic-Rule-Violation-Detection-System-master/IMG_4659-output.MP4"
...
def main(sess=''):
    if True:
        ...
        cv2.line(frame, (int(xl1),int(yl1)), (int(xl2),int(yl2)), (0,400,0),1)
