...
def checkWrongLane(data_deque, direction, obj_name):
	...
	polygon_person = Polygon(polygons[2])
	...
	if not obj_name == 'person':
		...
		if polygon_person.contains(point):
			return "Warning: Wrong Lane for person"
		else:
			return False
...
# draw the bounding box and the speed of the object
count_vehicle = 0
def draw_boxes(img, bbox, names, object_id, detections, cur_time, identities=None, offset=(0, 0)):
	...
	for i, box in enumerate(bbox):
		...
		if len(data_deque[id]) >= 2:
			...
			if len(data_deque[id]) >= 6:
				...
				# for x in range(len(detections.class_id)) :
				# 	if detections.class_id[x] == 0:
				# 		check = 0
				# 		for y in range(len(detections.class_id)) :
				# 			if detections.class_id.class_id[y] == 3:
...
filename="/Users/acemitto/Desktop/video 2 person 2-Output.mp4"
...
def main(sess=''):
	if True:
		...
		selected_classes = [0, 1, 2, 3, 5, 7, 9]
		...
		if len(outputs) > 0:
			...
			frame = draw_boxes(frame, bbox_xyxy, model.names, object_id, detections, cap.get(cv2.CAP_PROP_POS_MSEC), identities)
