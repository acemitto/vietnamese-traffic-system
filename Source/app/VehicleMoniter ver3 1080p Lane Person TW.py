...
def UI_box(x, img, kmh, obj_name, data_deque, red_light, color=None, label=None, id=id, line_thickness=None):
...
def draw_boxes(img, bbox, names, object_id, cur_time, identities=None, offset=(0, 0)):
	...
	for i, box in enumerate(bbox):
		...
		if len(data_deque[id]) >= 2:
			...
			UI_box(box, img, speed_check, obj_name, data_deque[id], red_light, label=label, color=color, id=id ,line_thickness=1)
...
polygons = [
	np.array([
		[821, 606],[1127, 606],[1233, 874],[823, 874]
	]),
	np.array([
		[1136, 606],[1447, 606],[1665, 874],[1246, 874]
	])
]

polygons_person = [
	np.array([
		[273, 580],[1414, 580],[1813, 1080],[-34, 1080]
	])
]
# polygons = [ np.array([
...
cap=cv2.VideoCapture('/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/5. VIDEO/0. SOURCES VIDEO/video 2 person 2.mp4') # 0 stands for very first webcam attach
...
# filename="/Users/acemitto/Desktop/video 3 person-output2.mp4"
...
zone_annotators_person = [
	sv.PolygonZoneAnnotator(
		zone=zone_person, 
		color=colors.by_idx(3),
		thickness=0
	)
	for index_person, zone_person in enumerate(zones_person)
]

box_annotators_person = [
	sv.BoxAnnotator(
		color=colors.by_idx(3), 
		thickness=1, 
		text_thickness=1, 
	...
	if True:
		...
		for i, (zone, zone_annotator, box_annotator) in enumerate(zip(zones, zone_annotators, box_annotators)):
			...
			for x in range(len(detections_filtered.class_id)) :
				if i == 1:
					if detections_filtered.class_id[x] != 3 and detections_filtered.class_id[x] != 1:
						detections_check.class_id.append(detections_filtered.class_id[x])
						detections_check.xyxy.append(detections_filtered.xyxy[x])

			frame = box_annotator.annotate(scene=frame, detections=detections_check, skip_label=False, type=1)
			frame = zone_annotator.annotate(scene=frame)
			
		for zone_person, zone_annotator_person, box_annotator_person in zip(zones_person, zone_annotators_person, box_annotators_person):
			# annotate
			mask_person, idd = zone_person.trigger(detections=detections)
			#print(mask, idd)
			detections_filtered_person = detections[mask_person]
			detections_check_person = detections[mask_person]
			detections_check_person.class_id = []
			detections_check_person.xyxy = []
			
			for x in range(len(detections_filtered_person.class_id)) :
				if detections_filtered_person.class_id[x] == 0:
					check = 0
					for y in range(len(detections_filtered_person.class_id)) :
						if detections_filtered_person.class_id[y] == 3:
							if overlap2(np.array(detections_filtered_person.xyxy[x]), np.array(detections_filtered_person.xyxy[y])) :
								check = check + 1

					if check == 0 :
						detections_check_person.class_id.append(detections_filtered_person.class_id[x])
						detections_check_person.xyxy.append(detections_filtered_person.xyxy[x])
						

			frame = box_annotator_person.annotate(scene=frame, detections=detections_check_person, skip_label=False, type=0)
			frame = zone_annotator_person.annotate(scene=frame)
