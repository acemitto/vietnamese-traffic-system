...
def checkWrongLane(data_deque, direction):
	...
	polygon_stay = Polygon(polygons_parking[0])
	polygon_road = Polygon(polygons_road[0])
	...
	if direction == 'down':
		...
		if polygon_stay.contains(point) != True:
			if polygon_road.contains(point):
...
polygons = [
	np.array([
		[791, 238],[892, 238],[506, 1080],[0, 1080]
	]),
	np.array([
		[896, 238],[992, 238],[985, 1080],[529, 1080]
	]),
	np.array([
		[998, 238],[1114, 238],[1569, 1080],[1008, 1080]
	])
]

polygons_parking = [
	np.array([
		[1108, 227],[1175, 227],[1346, 450],[1008, 450]
	])
]

polygons_road = [
	np.array([
		[1108, 227],[1175, 227],[1920, 1080],[1569, 1080]
	])
]
# polygons = [ np.array([ 
...
cap=cv2.VideoCapture('/Users/acemitto/Desktop/IMG_5293.mp4') # 0 stands for very first webcam attach
...
filename="/Users/acemitto/Desktop/IMG_5293-output.mp4"
	...
	if True:
		...
		for i, (zone, zone_annotator, box_annotator) in enumerate(zip(zones, zone_annotators, box_annotators)):
			...
			for x in range(len(detections_filtered.class_id)) :
				if i == 0:
					if detections_filtered.class_id[x] == 3 or detections_filtered.class_id[x] == 1:
						detections_check.class_id.append(detections_filtered.class_id[x])
						detections_check.xyxy.append(detections_filtered.xyxy[x])
				if i == 2:
					if detections_filtered.class_id[x] == 2 or detections_filtered.class_id[x] == 5 or detections_filtered.class_id[x] == 7:
						detections_check.class_id.append(detections_filtered.class_id[x])
						detections_check.xyxy.append(detections_filtered.xyxy[x])
