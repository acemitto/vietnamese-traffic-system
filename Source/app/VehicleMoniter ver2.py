...
def get_plate(image_path, Dmax=500, Dmin = 801):
...
def getLicensePlateNumber(url):
	...
	if cor != '' :
		...
		display5.configure(text="29C039.45")
		print("no plate!")
	...
	for v in vehicles:
		if v.crossed==False and len(v.points)>=2:
			...
			if y1>yl3 and y2<yl3:
				...
				name='/Users/acemitto/Desktop/STUDY/ACE/STOCK/testData/Source/Traffic-Rule-Violation-Detection-System-master/Rule Breakers/culprit'+str(time.time())+'.jpg'
	...
	for v in vehicles:
		if v.speedChecked==False and len(v.points)>=2:
			...
			if y2<yl1 and y2>yl3 and v.entered==False:
				...
				print(speed, v.exitTime, v.enterTime)
				bimg=img[int(v.rect[1]):int(v.rect[1]+v.rect[3]), int(v.rect[0]):int(v.rect[0]+v.rect[2])]
				frame2=bimg
				img2 = Image.fromarray(frame2)
				w,h=img2.size
				...
				display2.imgtk = imgtk2 #Shows frame for display 1
				display2.configure(image=imgtk2)
				display3.configure(text=str(speed)[:5]+'Km/h')
