...
wpod_net_path = '/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/vietnamese-traffic-system/Source/Traffic-Rule-Violation-Detection-System-master/wpodnet/weights/wpodnet.pth'
...
def UI_box(x, ori_img, img, kmh, obj_name, data_deque, wrong_lane, no_helmet, color=None, label=None, id=id, line_thickness=None):
	tl = line_thickness
...
def draw_boxes(ori_img, img, bbox, names, object_id, cur_time, identities=None, offset=(0, 0)):
	...
	for i, box in enumerate(bbox):
		...
		if len(data_deque[id]) >= 2:
			...
			UI_box(box, ori_img, img, speed_check, obj_name, data_deque[id], wrong_lane, no_helmet, label=label, color=color, id=id ,line_thickness=3)
...
model_helmet = YOLO('hemletYoloV8_100epochs.pt')  # load a pretrained model (recommended for training)
...
polygons_lane = [
	np.array([
		[830, 400],[3037, 400],[3880, 2160],[-100, 2160]
	])
]

polygons = [
	np.array([
		[830, 400],[1544, 400],[1135, 2160],[-100, 2160]
	]),
	np.array([
		[1580, 400],[2280, 400],[2520, 2160],[1245, 2160]
	]),
	np.array([
		[2320, 400],[3037, 400],[3880, 2160],[2590, 2160]
	])
]

cap=cv2.VideoCapture('/Users/acemitto/Desktop/STUDY/ACE/CAPSTONE/5. VIDEO/0. SOURCES VIDEO/A001_07101641_C019.mp4') # 0 stands for very first webcam attach
...
filename='/Users/acemitto/Desktop/YOLOv9 wrong direction.mp4'
